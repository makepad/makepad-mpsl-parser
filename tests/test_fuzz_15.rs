...
const vec3 _CharacterMainColor = vec3(0.60294, 0.1515, 0.062067) ;
const vec3 _CharacterTerrainCol = vec3(0.35294, 0.16016, 0.12197) ;
const vec3 _CharacterCloakDarkColor = vec3(0.25735, 0.028557, 0.0056769) ;
const vec3 _CharacterYellowColor = vec3(0.88971, 0.34975, 0) ;
const vec3 _CharacterWhiteColor = vec3(0.9928, 1, 0.47794) ;
...
const vec3 _CloudCol = vec3(1, 0.84926, 0.69853) ;
const vec3 _BackCloudCol = vec3(0.66176, 0.64807, 0.62284) ;
const vec3 _CloudSpecCol = vec3(0.17647, 0.062284, 0.062284) ;
const vec3 _BackCloudSpecCol = vec3(0.11029, 0.05193, 0.020275) ;
const float _CloudFogStrength = 0.50000 ;
const vec3 _TombMainColor = vec3(0.64706, 0.38039, 0.27451) ;
const vec3 _TombScarfColor = vec3(0.38971, 0.10029, 0.10029) ;
...
const vec3 _TerrainSpecColor = vec3(1, 0.77637, 0.53676) ;
...
const vec3 _TerrainShadowColor = vec3(0.48529, 0.13282, 0) ;
const vec3 _TerrainDistanceShadowColor = vec3(0.70588, 0.4644, 0.36851) ;
const float _TerrainDistanceShadowPower = 0.11000 ;
const vec3 _FlyingHelperMainColor = vec3(0.85294, 0.11759, 0.012543) ;
const vec3 _FlyingHelperCloakDarkColor = vec3(1, 0.090909, 0) ;
const vec3 _FlyingHelperYellowColor = vec3(1, 0.3931, 0) ;
const vec3 _FlyingHelperWhiteColor = vec3(1, 1, 1) ;
...
vec2 sdFace( vec3 pos, vec2 currentDistance )
{
    ...
    return smin_mat( vec2(face, 54.0), vec2(character,currentDistance.y), 0.01, 0.2 );
}
vec2 sdCharacter( vec3 pos )
{
    ...
    vec2 mainCloakMat = vec2(mainCloak, 51.0 );
    float headScarf = sdHeadScarf(pos);
    vec2 headScarfMat = vec2(headScarf, 52.0 );
    float longScarf = sdScarf(pos);
    vec2 longScarfMat = vec2( longScarf, 53.0 );
    headScarfMat = smin_mat( headScarfMat, longScarfMat, 0.02, 0.1 );
    float head = sdHead( pos );
    vec2 headMat = vec2( head, 50.0 );
    ...
    vec2 legsMat = vec2( sdLegs(pos), 50.0 );
...
vec2 sdClouds( in vec3 pos )
{
   ...
   frontClouds = min(mainPlane * _CloudGlobalParams.x, frontClouds);
    ...
    return min_mat(vec2(frontClouds,21.0), vec2(backClouds,20.0));
}
...
vec2 sdFlyingHelpers( vec3 pos )
{
 ...
 vec2 helperMat = smin_mat( vec2(helper,40.0), vec2(helperScarf,41.0), 0.01, 0.1);
...
vec2 sdDesert( in vec3 pos, in float terrain )
{
    ...
    float mat = 9.0;
    return vec2( pos.y + terrain, 10.0 );
}
...
{
    vec3 scarfPos = pos - scarfOffset;
    scarfPos = (vec4(scarfPos,1.0)*_TombScarfMat).xyz;
...
{
 vec2 mainTomb = vec2(_DrawDistance, 30.0);
 for( float t = -1.0; t <= 1.0; t += 2.0 )
 {
  ...
  vec2 tTombMat = min_mat( vec2(tTomb, 30.0), vec2(tombScarf, 31.0));
...
vec2 map( in vec3 pos )
{
 ...
 vec2 res = character;
    if( res.x > 0.01 )
    {
     ...
     vec2 trail = vec2(-sdCharacterTrail(pos, desert), 11.0 );
  ...
  res = min_mat( res, terrain);
   ...
   vec2 tombs = sdTombs(pos);
            ...
            vec2 pyramid = vec2(sdBigMountain(pos), 1.0);
...
vec3 render( in vec3 ro, in vec3 rd )
{
 ...
 if( m < (11.0 + 0.1) )
 {
  ...
  float rim = (1.0 - clamp( dot( nor , -rd ), 0.0, 1.0 ));
  rim = clamp( pow( rim, _TerrainRimPower), 0.0, 1.0 ) *_TerrainRimStrength ;
 ...
 if( m < (21.0 + 0.1) )
 {
  ...
  vec3 inCloudCol = mix(cloudCol, _BackCloudCol + skyCol * 0.5 + spec * _BackCloudSpecCol, 21.0 - m);
  return mix( inCloudCol , skyCol, skyFog * _CloudFogStrength );
 }
 if( m < (31.0 + 0.1) )
 {
  float diff = clamp(dot(nor,_LightDir) + 1.0, 0.0, 1.0);
  vec3 col = mix( _TombMainColor, _TombScarfColor * 2.0, m - 30.0 );
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
 ...
 vec3 rd = normalize(vec4(screenRay,0.0)*_CameraInvViewMatrix).xyz;
